...
class LocalMigrationScheduler:
    ...
    #get_migrate_out_requests_reroute

    def get_redispatch_requests(self,) -> List[LlumnixRequest]:
        migrate_out_requests = self._get_pending_requests()
    ...
    def _filter_waiting_queue_reroute(self, waiting):
        filtered_waiting = [request for request in waiting]
        #if not batched:
        filtered_waiting.sort(key=lambda x: x.pending_time, reverse=True)
    ...
    def _get_pending_requests(self) -> List[LlumnixRequest]:
        waiting: Deque[LlumnixRequest] = self.backend_engine.get_waiting_queue()
        filtered_waiting = self._filter_waiting_queue_reroute(waiting)
        return filtered_waiting if filtered_waiting else []

    def _get_pending_most_request(self, batched=False, active=False) -> List[LlumnixRequest]:
        waiting: Deque[LlumnixRequest] = self.backend_engine.get_waiting_queue()
        filtered_waiting = self._filter_waiting_queue_reroute(waiting, batched=batched)
        if not active:
            return filtered_waiting[0] if filtered_waiting else None
        else:
            if not batched:
                return [filtered_waiting[0]] if filtered_waiting else []
            else:
                return filtered_waiting if filtered_waiting else []
