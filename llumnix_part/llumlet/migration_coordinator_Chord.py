...
class MigrationCoordinator:
    ...
    async def migrate_out_batched_waiting_requests(self, migrate_out_requests):
        successful_requests = self.backend_engine.remove_batched_waiting_requests(migrate_out_requests)
    ...
    def migrate_in_pre_alloc(self,
        ...
        if len(pre_alloc_blocks) != block_num:
            print('fail to alloc, required {}, but only {} allocated'.format(block_num, len(pre_alloc_blocks)))
