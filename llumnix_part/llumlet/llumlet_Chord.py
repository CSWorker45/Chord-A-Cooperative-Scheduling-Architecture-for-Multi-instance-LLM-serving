...
class Llumlet:
    ...
    async def get_redispatch_requests(self,) -> List[LlumnixRequest]:
        redispatch_requests = self.migration_scheduler.get_redispatch_requests()
        return redispatch_requests

    async def reroute_batched(self, target_request_ids, dst_instance_id: str, dst_instance_actor_handle: ray.actor.ActorHandle, gather=False) -> List[str]:
        # in the reroute, the target_requests should be set.

        if not target_request_ids:
            raise ValueError("No target requests provided for migration out, and it should not be this way.")

        # for migrate_out_request in target_requests:
        #     migrate_out_request.is_migrating = True

        if not gather:
            logger.info("Logical master instance {} reroute out request to {}".format(self.instance_id, dst_instance_id))
        else:
            logger.info("Branch instance {} reroute out preempted requests to master instance {}".format(self.instance_id, dst_instance_id))

        migrated_request_list = await self._migrate_out_batched_waiting_requests(target_request_ids, dst_instance_id, dst_instance_actor_handle)
        

        return migrated_request_list

    async def _migrate_out_batched_waiting_requests(self, migrate_out_request_ids, dst_instance_id, dst_instance_actor_handle: ray.actor.ActorHandle):
        try:
            t0 = time.time()
            logger.info("{}->{} begin prefill reroute".format(self.instance_id, dst_instance_id))            
            successful_requests = await self.migration_coordinator.migrate_out_batched_waiting_requests(migrate_out_request_ids)

            if len(successful_requests) != 0:
                await dst_instance_actor_handle.execute_engine_method.remote("commit_dst_requests_batched", successful_requests)            
                
            t1 = time.time()
            logger.info("Instance {}->{} migrate done, prefill reroute {} requests, cost: {} ms" \
                        .format(self.instance_id, dst_instance_id, len(successful_requests), (t1 - t0)*1000))
            successful_request_ids = [request.request_id for request in successful_requests]
    ...
    async def migrate_out_reroute(self, dst_instance_id: str, dst_instance_actor_handle: ray.actor.ActorHandle, batched=False, active=True, target_requests=None) -> List[str]:
        if not target_requests:
            migrate_out_requests = self.migration_scheduler.get_redispatch_requests(batched=batched, active=active)
        else:
            migrate_out_requests = target_requests
        ...
        for migrate_out_request in migrate_out_requests:
                ...
                if not batched and len(migrated_request) == 0 and migrate_out_request.eom:
                    break
    ...
    async def _migrate_out_one_request(self,
        ...
        try:
            ...
            if migrate_out_request.status == RequestStatus.RUNNING:
                ...
                logger.info("Instance {}->{} migrate done, prefill reroute request {}, migration status: {}, len: {} blocks, cost: {} ms" \
                            .format(self.instance_id, dst_instance_id, migrated_request, status, \
                                    sum(migrate_out_request.stage_num_blocks_list), (t1 - t0)*1000))
        except ray.exceptions.RayActorError:
            logger.info("Instance {} is dead.".format(dst_instance_id))
