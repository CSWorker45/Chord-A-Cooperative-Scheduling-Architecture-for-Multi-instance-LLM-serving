...
class Manager:
    def __init__(self,
        ...
        # pre-tokenizer for dispatching.
        print('engine args in the manager init:', engine_args)

        print('Load pre_tokenizer for manager...')
        self.pre_tokenizer = get_tokenizer(engine_args.model)
        self.block_size = engine_args.block_size
        if engine_args is not None and instance_args is None:
            engine_args = None
    ...
    async def generate(self, request_id: str, server_info: ServerInfo, *args, **kwargs,) -> None:
        ...
        # we should add the part that accesses the request's memory consumption by far.
        prompt_ = args[0]
        prompt_len = len(self.pre_tokenizer(prompt_).input_ids)
        req_n_blocks = (prompt_len + self.block_size - 1) // self.block_size
        
        instance_id, request_expected_steps = self.global_scheduler.dispatch(req_n_blocks)
    ...
    async def _push_migrations(self) -> None:
        if self.enable_pd_disagg:
            ...
            asyncio.create_task(self._migrate(PairMigrationConstraints.PREFILL_REROUTING))
    ...
    async def _migrate(self, pair_migration_type: PairMigrationConstraints) -> None:
        ...
        async def migrate_done_callback(ret, migrate_instance_pair: Tuple[str, str], master_instance=None) -> None:
            ...
            if isinstance(ret, (ray.exceptions.RayActorError, ray.exceptions.RayTaskError, KeyError)):
                ...
                if migrate_out_request_ids:
                    for migrate_out_request_id in migrate_out_request_ids:
                        self.request_instance[migrate_out_request_id] = migrate_instance_pair[1]
                if not self.manager_args.enable_pd_disagg:
                    if pair_migration_type != PairMigrationConstraints.PREFILL_REROUTING:
                        ...
                        if migrate_instance_pair[0] != master_instance: 
                            logger.info("Instance {}->{} branch to master done, prefill rerouting {} request".format(
                                migrate_instance_pair[0], migrate_instance_pair[1], len(migrate_out_request_ids)))
                        else:
                            logger.info("Instance {}->{} master to branch done, prefill rerouting {} request".format(
                                migrate_instance_pair[0], migrate_instance_pair[1], len(migrate_out_request_ids)))

        def migrate_done_callback_wrapper(migrate_instance_pair: Tuple[str, str], fut, master_instance=None) -> None:
            ret = fut.result()[0]
            loop = asyncio.get_event_loop()
            loop.create_task(migrate_done_callback(ret, migrate_instance_pair, master_instance=master_instance))

        try:
            migration_tasks = []
            # if pair_migration_type == PairMigrationConstraints.PREFILL_REROUTING:
            src_instance_ids = self.global_scheduler.get_redispatch_src_instances()
            if src_instance_ids is not None:
                master_src_instance_id = src_instance_ids[0]
                for src_instance_id in src_instance_ids:
                    target_requests = await self.instances[src_instance_id].get_redispatch_requests.remote()
                    if src_instance_id != master_src_instance_id:
                        # redispatch the preempted requests to the master src instance to realize central control logically.
                        # This is one to one, but all preempted requests from branch instance would be redispatched.
                        if len(target_requests) != 0:
                            target_request_ids = self.global_scheduler.post_process(target_requests)
                            dst_instance_id = master_src_instance_id
                            if not(self.instance_migrating[src_instance_id] or self.instance_migrating[dst_instance_id]):
                                ...
                                migrate_instance_pair = (src_instance_id, dst_instance_id)
                                task = asyncio.gather(self.instances[src_instance_id].reroute_batched.remote(target_request_ids,
                                                dst_instance_id, dst_instance_actor_handle), return_exceptions=True)
                                task.add_done_callback(partial(migrate_done_callback_wrapper, migrate_instance_pair, master_instance=master_src_instance_id))
                                migration_tasks.append(task)
                    else:
                        # Redispatch the most pending requests from the master src instance to a desired dst instance.
                        if len(target_requests) != 0:
                            candidate_instance_infos = self.global_scheduler.get_redispatch_dst_infos()
                            redispatch_plans = self.global_scheduler.derive_redispatching_plans(master_src_instance_id, target_requests, candidate_instance_infos)
                            src_instance_id = master_src_instance_id
                            for dst_instance_id, target_request_ids in redispatch_plans.items():
                                if not(self.instance_migrating[src_instance_id] or self.instance_migrating[dst_instance_id]):
                                    self.instance_migrating[src_instance_id] = True
                                    self.instance_migrating[dst_instance_id] = True
                                    dst_instance_actor_handle = self.instances[dst_instance_id]
                                    migrate_instance_pair = (src_instance_id, dst_instance_id)
                                    task = asyncio.gather(self.instances[src_instance_id].reroute_batched.remote(target_request_ids,
                                                        dst_instance_id, dst_instance_actor_handle, gather=True), return_exceptions=True)
                                    task.add_done_callback(partial(migrate_done_callback_wrapper, migrate_instance_pair, master_instance=master_src_instance_id))
                                    migration_tasks.append(task)
            else:
                pair_migration_type = PairMigrationConstraints.NO_CONSTRAINTS
            ...
            if len(migration_tasks) > 0 and not self.manager_args.enable_pd_disagg:
                if pair_migration_type != PairMigrationConstraints.PREFILL_REROUTING:
                    ...
                    logger.info("{} prefill reroute tasks starts.".format(len(migration_tasks)))
            await asyncio.gather(*migration_tasks, return_exceptions=True)
            if len(migration_tasks) > 0 and not self.manager_args.enable_pd_disagg:
                if pair_migration_type != PairMigrationConstraints.PREFILL_REROUTING:
                    logger.info("{} migration tasks ends.".format(len(migration_tasks)))
                else:
                    logger.info("{} prefill reroute tasks ends.".format(len(migration_tasks)))
    # pylint: disable=W0703
        except Exception as e:
            logger.error("Unexpected exception: {}".format(e))
            logger.error("Exception traceback: {}".format(traceback.format_exc()))
