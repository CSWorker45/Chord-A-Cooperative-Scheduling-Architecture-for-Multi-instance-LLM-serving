...
class BackendVLLM(BackendInterface):
    ...
    def commit_dst_requests_batched(self, backend_requests: List[SequenceGroupLlumnix]) -> None:
        """Commit the dst requests after migration."""
        if not backend_requests:
            return
        for backend_request in backend_requests:
            self.commit_dst_request(backend_request)

    def commit_dst_request(self, backend_request: SequenceGroupLlumnix) -> None:
        seq = backend_request.get_seqs()[0]
        seq.seq_id = next(self.engine.seq_counter)
        print("commit_dst_request: {}, status: {}".format(backend_request.request_id, backend_request.status))
        if backend_request.status != RequestStatus.WAITING:
            # print('backend_request id:{}, backend_request status:{} in dst engine'.format(backend_request.request_id, backend_request.status))
            # print("pop request {} from pre_alloc_cache_dict".format(backend_request.request_id))
        ...
        if backend_request.status == RequestStatus.WAITING:
            ...
            backend_request.set_status(RequestStatus.WAITING)
    ...
    def remove_batched_waiting_requests(self, *args, **kwargs):
        return self.engine.scheduler[0].remove_batched_waiting_requests(*args, **kwargs)
