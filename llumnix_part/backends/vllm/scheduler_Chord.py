...
class SchedulerLlumnix(Scheduler):
    ...
    def remove_waiting_request(self, request_id: str) -> bool:
        for seq_group in self.waiting:
            if seq_group.request_id == request_id and \
               seq_group.get_seqs()[0].n_blocks * self.cache_config.block_size <= self._get_prompt_limit(seq_group):
                self.waiting.remove(seq_group)
                #logger.info("request: {}, _status:{}, status: {} in scheduler before".format(seq_group.request_id, seq_group._status, seq_group.status))
                #seq_group.set_status(RequestStatus.WAITING_MIGRATING)
                #logger.info("request: {}, _status:{}, status: {} in scheduler after".format(seq_group.request_id, seq_group._status, seq_group.status))
                return True
        return False
    ...
    def _allocate_and_set_running(self, seq_group: SequenceGroup) -> None:
        ...
        if seq_group.status == RequestStatus.WAITING_MIGRATING:
            ...
            # not in our implementation. Since we are implementing redispatching (Eddie.)
    ...
    def _get_instance_info(self, scheduled_seq_groups: List[SequenceGroupLlumnix]) -> InstanceInfo:
        ...
        #has_over_pending_requests = False
        max_pending = 0
        sum_pending = 0
        if self.waiting:
            ...
            for seq_group in self.waiting:
                ...
                num_blocks = math.ceil(num_prompt_tokens / self.cache_config.block_size)
                sum_pending += seq_group.pending_time
                max_pending = max(max_pending, seq_group.pending_time)
