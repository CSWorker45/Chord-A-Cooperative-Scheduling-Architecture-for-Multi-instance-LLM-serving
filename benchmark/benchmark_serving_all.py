    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        ...
        try:
            async with session.post(f'http://{ip_ports[server_id]}/generate_benchmark', json=request) as resp:
                ...
                print("num_finished_requests: {}".format(num_finished_requests))
...
def calculate_throughput(queries,
    ...
    print(f'throughput_tok_s {throughput_tok_s:.04f}')
...
class MeasureLatency:
    def __init__(self):
        ...
        self._tail_decode_token_latencies = []
    ...
    def measure(self, f):
        async def measured(*args, **kwargs):
            ...
            if 'per_token_latency' in output:
                ...
                tail_decode_token_latency = 0 if len(lat_arr) == 1 else np.max(lat_arr[1:,1])
                decode_sum_latency = 0 if len(lat_arr) == 1 else np.sum(lat_arr[1:,1])
                self._tail_decode_token_latencies.append(tail_decode_token_latency)
...
async def benchmark(
    ...
    print('P99 prefill latency:', np.percentile(m._prefill_token_latencies, 99))# len(m._prefill_token_latencies), m._prefill_token_latencies)
    print('P50 prefill latency:', np.percentile(m._prefill_token_latencies, 50))
    print('Mean prefill latency:', np.mean(m._prefill_token_latencies))
    print('\n')
    print('P99 decode latency:', np.percentile(m._tail_decode_token_latencies,99))# m._tail_decode_token_latencies)
    print('P50 decode latency:', np.percentile(m._tail_decode_token_latencies,50))
    print('Mean decode latency:', np.mean(m._tail_decode_token_latencies))
...
def main():
    ...
    parser.add_argument('--coefficient_variation', type=float, default=1.6)
    ...
    group.add_argument('--dataset_type', type=str, choices=['sharegpt', 'burstgpt', 'lmsys'])
    ...
    if args.dataset_type:
        ...
        if args.dataset_type=='sharegpt':
            dataset_path = '../datasets/sharegpt_sample.json'
        elif args.dataset_type=='burstgpt':
            dataset_path = '../datasets/burstgpt_sample.json'
        elif args.dataset_type=='lmsys':
            dataset_path = '../datasets/lmsys_chat_1m_sample.json'
